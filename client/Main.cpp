...
DWORD WINAPI ClientThread(LPVOID sock) {
    ...
    while (1) {
        ...
        if (packettype == Pack)
        {
            ...
            recv(Con, (char*)&msg_size, sizeof(int), NULL); char* msg = new char[msg_size + 1];
            msg[msg_size] = '\0';
    ...
    closesocket(Connection); 	return 0;
}
...
int main() {    // with threads
	...
	if (!mysql_real_connect(conn, "localhost", "user_me", "maximum_yt1488", "chat_base", 3306, NULL, 0)) {
        ...
        if (sign_up_in == 1) {
            ...
            if (sign_up_in == 2) {
                cout << "Введите новые логин и пароль длинной в 8 знаков\nПароль должен содержать только буквы, цифры и знак _ \nБез специальных знаков\n(. , ? : ! ~ * % ^ & и т.д.)\n";
                bool login_ok = false;
                while (!login_ok) {
                    cout << "Логин: ";
                    cin >> login;
